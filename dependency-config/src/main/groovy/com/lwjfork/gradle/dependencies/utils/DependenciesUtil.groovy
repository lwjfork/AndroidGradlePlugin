...
import com.lwjfork.gradle.utils.DependencyUtil
...
class DependenciesUtil {
    ...
    static def dependencySysName(Project project, String buildTypeName) {
        if (buildTypeName == null || buildTypeName.length() == 0) {
            "${DependencyUtil.getDependencyModePrefix(project)}"
        } else {
            "$buildTypeName${DependencyUtil.getDependencyModePrefix(project).capitalize()}"
        }
